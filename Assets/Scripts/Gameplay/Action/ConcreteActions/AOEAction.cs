    ...
    public class AOEAction : Action
    {
        ...
        public override bool OnUpdate(ServerCharacter clientCharacter)
        {
            ...
            else
            {
                ...
                {
                    if (m_TimeElapsed >= Config.ExecTimeSeconds)
                        PerformAoE(clientCharacter);
                }
        ...
        private void PerformAoE(ServerCharacter parent)
        {
            ...
            var affectData = Config.ActionAffectData;
            var colliders = Physics.OverlapSphere(m_Data.Position, Config.Radius, LayerMask.GetMask(affectData.AffectLayer));
            for (var i = 0; i < colliders.Length; i++)
            {
                affectData.AffectValue(parent, colliders[i].gameObject, -Config.Amount);
            }
        }
